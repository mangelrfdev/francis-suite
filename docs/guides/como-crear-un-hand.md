...
> El motor no se reescribe: registrás la lógica, la probás, regenerás el schema y listo.

Francis Suite ya es **funcional** para pipelines reales. Cuando necesitás una capacidad nueva — leer un formato, llamar a un servicio, transformar datos — la sumás como **hand** y el resto del framework (parser, runtime, boxes, records, expresiones) sigue igual.
...
El runtime solo conoce los hands que se importan al cargar el paquete. Agregá **una línea** en [`francis_suite/hands/core/__init__.py`](../../francis_suite/hands/core/__init__.py):
...
Los tests aseguran que el hand se comporta como documentás. Patrón mínimo (igual que en [`tests/test_pipeline.py`](../../tests/test_pipeline.py)):
...
**Commitear** junto con el código: `mi_hand.py`, `__init__.py`, test y archivos en `schema/` si regeneraste.

---

## Reglas que no podés saltear
...
| **2 — Scoping** | `while` y `loop` **no** abren scope nuevo; `function-call` **sí**. Si no tocás una box, no cambia. |
...
*¿Dudas con un hand concreto? Mirá un hand parecido en `francis_suite/hands/core/` y copiá su estructura — el framework ya tiene decenas de ejemplos reales.*
