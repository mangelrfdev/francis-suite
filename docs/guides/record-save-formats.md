...
**Implementación actual (incl. xml, html, txt, excel, parquet), `record-journal` (NDJSON incremental), `record-export-*` / `record-export-root-attr` / `show-attribute`, `record-xml-*` (solo XML), **`clean-data`**, forma de export por defecto y opciones **`allow-nested` / `allow-prefix` / `allow-sufix`:** sección *Forma de los datos exportados* en [record-save.md](record-save.md) (`examples/demos/books_all_pages.xml`, `examples/demos/all_books_pages.xml`).
